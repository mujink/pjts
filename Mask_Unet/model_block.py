from tensorflow.keras.layers import Conv2D, BatchNormalization, ZeroPadding2D, Convolution2DTranspose
...
def con2dTrans(input_layer, filters_shape, up_sample=False, activate="l", bn=True):
    """
    =================================================
    filters_shape : len > 2
    init:
    ...
    conv = Convolution2DTranspose(filters=filters_shape[-1], kernel_size = filters_shape[0], strides=strides, padding=padding,
                                  use_bias=not bn, kernel_regularizer=tf.keras.regularizers.l2(0.0005),
                                  kernel_initializer=tf.random_normal_initializer(stddev=0.01),
                                  bias_initializer=tf.constant_initializer(0.))(input_layer)
